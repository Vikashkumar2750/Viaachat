...
const NavItem: React.FC<{
  ...
  <button onClick={onClick} className="flex flex-col items-center justify-center flex-1 transition-all relative group py-1">
    <div className={`p-2 px-5 rounded-full transition-all duration-200 ${active ? 'bg-emerald-500/15' : 'bg-transparent group-hover:bg-white/8'}`}>
    ...
    {badge && badge > 0 && (
      <span className="absolute top-1 right-3 bg-emerald-500 text-white text-[10px] font-bold min-w-[18px] h-[18px] flex items-center justify-center rounded-full border-2 border-gray-950 shadow-sm">
        {badge > 9 ? '9+' : badge}
      </span>
    )}
  </button>
